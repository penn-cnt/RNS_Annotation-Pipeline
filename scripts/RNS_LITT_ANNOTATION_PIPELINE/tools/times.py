...
def timestring_to_timestamp_annotation(timestring, time_delta=0.0):
    """
    :param timestring: string - time string
    :param time_delta: float - time offset in second
    :return: int - datetime timestamp
    """
    time = datetime.strptime(timestring, "%m/%d/%y %H:%M:%S.%f") + timedelta(seconds=time_delta)
    return int(time.replace(tzinfo=timezone.utc).timestamp() * 1e6)
...
def find_closest_event_ind(value, array, threshold = 0, from_side = 'right'):
    '''
    ...
    if from_side == 'left':
        valid_idx = np.where(diff >= threshold)[0]
        out = valid_idx[diff[valid_idx].argmin()]
    return out


def timestamp_to_ind(timestamp, catalog,thres = 0.5e6):
    """
...
def extact_timestamp_to_ind(timestamp, catalog, criteria = 'start'):
    if criteria == 'start':
        catalog_timestamps = catalog['Start UTC Timestamp']
    else:
        catalog_timestamps = catalog['End UTC Timestamp']

    ind = np.where(catalog_timestamps == timestamp)[0]
    if len(ind) == 1:
        return ind[0]
